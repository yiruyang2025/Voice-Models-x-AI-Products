...
```
# Cell 2.5: Plot log-Mel Spectrograms with Frequency Annotations (per class)
...
def plot_log_mel_with_annotations(filepath, title="Audio Sample"):
    y, sr = librosa.load(filepath, sr=48000)  # Use fixed 48kHz
    ...
    # Estimate fundamental frequency index
    ...
    plt.title(f"{title} - 64-bin Log-Mel Spectrogram")

    # Annotate fundamental
    plt.axhline(y=f0_freq, color='cyan', linestyle='--', linewidth=1)
    plt.text(0.5, f0_freq + 200, "Fundamental (Main Tone)", color='cyan', fontsize=8)

    # Annotate overtone region (f0 + ~1.5~4kHz)
    overtone_start = f0_freq + 1500
    overtone_end = overtone_start + 3000
    plt.axhspan(overtone_start, overtone_end, color='yellow', alpha=0.2)
    plt.text(0.5, overtone_start + 1000, "Overtones / Resonance", color='goldenrod', fontsize=8)

    # Bottom caption
    plt.text(0, -8, 
             "Different cowbell sounds have different spectral distributions — the basis for classification.",
...
base_path = "cowbells"  # Original audio folder
class_dirs = sorted([d for d in os.listdir(base_path) if os.path.isdir(os.path.join(base_path, d))])

for class_name in class_dirs:
    print(f"\nClass: {class_name}")
    wav_files = glob(os.path.join(base_path, class_name, "*.wav"))
    if wav_files:
        plot_log_mel_with_annotations(wav_files[0], title=class_name)
    else:
