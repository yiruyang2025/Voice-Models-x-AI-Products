...
import os, pandas as pd, librosa, numpy as np, tensorflow as tf
...
def trim_audio(y):
    return y if len(y) == max_len else np.tile(y, int(np.ceil(max_len / len(y))))[:max_len]
...
swiss_files  = esc_meta_csv.query("category == 'cow'").filename.tolist()

for fn in swiss_files:
    wav = load_and_trim(f"ESC-50-master/audio/{fn}")
    X.append(extract_logmel(wav)); y.append(0)
print("Swiss originals :", sum(_ == 0 for _ in y))

# 1-b) Swiss augmentation (time-stretch + pitch-shift, 4× each)
for fn in swiss_files[:40]:
    base = load_and_trim(f"ESC-50-master/audio/{fn}")
    # time-stretch
    for rate in (0.8, 1.2):
        X.append(extract_logmel(trim_audio(librosa.effects.time_stretch(base, rate=rate)))); y.append(0)
    # pitch-shift
    for n_steps in ( 2, -2):
        X.append(extract_logmel(trim_audio(librosa.effects.pitch_shift(base, sr=sr, n_steps=n_steps)))); y.append(0)
print("Swiss after aug :", sum(_ == 0 for _ in y))

# 2) Urban originals
urb_meta = pd.read_csv("UrbanSound8K/metadata/UrbanSound8K.csv")
bell = urb_meta.query("classID == 6 and fold <= 3")
for _, r in bell.iterrows():
    wav = load_and_trim(f"UrbanSound8K/audio/fold{r.fold}/{r.slice_file_name}")
    X.append(extract_logmel(wav)); y.append(1)
print("Urban samples   :", sum(_ == 1 for _ in y))

# 3) Other: DIFFERENT augmentation params
for fn in swiss_files[:40]:
    base = load_and_trim(f"ESC-50-master/audio/{fn}")

    # time-stretch with different rates
    for rate in (0.7, 1.3):
        stretched = librosa.effects.time_stretch(base, rate=rate)   # ← add keyword
        X.append(extract_logmel(trim_audio(stretched))); y.append(2)

    # pitch-shift with ±4 semitones
    for n_steps in (4, -4):
        shifted = librosa.effects.pitch_shift(base, sr=sr, n_steps=n_steps)
        X.append(extract_logmel(trim_audio(shifted))); y.append(2)
print("Other samples   :", sum(_ == 2 for _ in y))
...
Urban samples   : 106
Other samples   : 160
Dataset shapes  : (466, 64, 469, 1) (466, 3)
```


```
# Cell 5 — Build, train and validate Lightweight CNN + Adapter (class weighting)
...
freq = np.bincount(y_train.argmax(axis=1))
class_weights = {i: freq.max() / f for i, f in enumerate(freq)}
...
model.compile(
    optimizer=optimizers.Adam(1e-3, weight_decay=1e-4),
