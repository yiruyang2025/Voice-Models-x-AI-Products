...
```
# Cell 2.5: Visualize Original and Augmented Audio Spectrograms + Sample Summary Table
...
def plot_log_mel_with_annotations(filepath, title="Audio Sample"):
    ...
    f0_index = np.argmax(S.mean(axis=1))
    f0_freq = mel_freqs[f0_index]

    plt.figure(figsize=(9, 3.8))
    librosa.display.specshow(S_dB, sr=sr, x_axis='time', y_axis='mel', cmap='magma')
    plt.colorbar(format="%+2.0f dB")
    plt.title(f"{title} - Log-Mel Spectrogram")

    # Fundamental frequency
    plt.axhline(y=f0_freq, color='cyan', linestyle='--', linewidth=1)
    plt.text(0.5, f0_freq + 500, "Fundamental", color='cyan', fontsize=8)

    # Overtone / resonance
    overtone_start = f0_freq * 2
    overtone_end = overtone_start + 2000
    overtone_start = min(overtone_start, mel_freqs[-1])
    overtone_end = min(overtone_end, mel_freqs[-1])
    plt.axhspan(overtone_start, overtone_end, color='yellow', alpha=0.25)
    plt.text(0.5, overtone_start + 800, "Overtones / Resonance", color='goldenrod', fontsize=8)

    # Caption
    plt.text(0, -8, "Spectral distribution varies across classes – basis for classification.",
...
# ============ Plot Samples and Count Per Class ============

base_dirs = {'Original': 'cowbells', 'Augmented': 'augmented'}
summary = []

for tag, base_path in base_dirs.items():
    print(f"\n=== {tag} Samples ===")
    if not os.path.exists(base_path):
        print(f"[Skipped] Folder not found: {base_path}")
        continue

    class_dirs = sorted([d for d in os.listdir(base_path) if os.path.isdir(os.path.join(base_path, d))])
    for class_name in class_dirs:
        files = glob(os.path.join(base_path, class_name, '*.wav'))
        summary.append({'Class': class_name, 'Type': tag, 'Count': len(files)})
        if files:
            print(f"Class: {class_name} ({tag})")
            plot_log_mel_with_annotations(files[0], title=f"{tag} - {class_name}")
        else:
            print(f"[Empty] No audio found for {class_name} ({tag})")

            # Cell 2.6: Visualize In-Memory Augmented Samples (from X and y)
...
label_names = sorted(os.listdir(out_dir))
...
for idx in range(len(X)):
    ...
    if class_name in shown_classes:
        continue  # Only show one per class

    mel = X[idx].squeeze()  # shape (64, 469)
    
    # Plot
    plt.subplot(2, 2, n_plots + 1)
...
df_summary = pd.DataFrame(summary).pivot(index="Class", columns="Type", values="Count").fillna(0).astype(int)
df_summary["Total"] = df_summary.sum(axis=1)

display(Markdown("### Sample Count Summary (Original + Augmented)"))
display(df_summary)
```
